...
// reorder for little endian
     ...
     * @param byteCipherData
     * @param pdwRoundKey
     * @param bytePlainData
     */
    public static void encryptall(byte[] bytePlainData,int[] pdwRoundKey,byte[] byteCipherData)
    {

        byte[] encbuffer= new byte[16];
        byte[] decbuffer= new byte[16];

//        final int nCnt = bytePlainData.length / 16;
//        final int nRemain = bytePlainData.length % 16;

        final int nCnt = byteCipherData.length / 16;
        final int nRemain = byteCipherData.length % 16;
        //log.debug("[SEED_KISA.encryptall()] nCnt = "+nCnt+", nRemain = "+nRemain);


        int nPos = 0;
        for(int i=0;i<nCnt;i++)
        {
            ByteUtil.bytecopy(bytePlainData,nPos,decbuffer,0,16);

            SEED_KISA.SeedEncrypt(decbuffer, pdwRoundKey, encbuffer);

            ByteUtil.bytecopy(encbuffer,0, byteCipherData, nPos, 16);
        ...
        {
            //나머지 nRemain byte처리
            ByteUtil.bytecopy(bytePlainData, nPos, decbuffer, 0, nRemain);
            SEED_KISA.SeedEncrypt(decbuffer, pdwRoundKey, encbuffer);
            ByteUtil.bytecopy(encbuffer,0, byteCipherData, nPos, nRemain);
        }

    }

    public static void decryptall(byte[] byteCipherData,int[] pdwRoundKey,byte[] bytePlainData)
    {
        ...
        for(int i=0;i<nCnt;i++)
        {
            ByteUtil.bytecopy(byteCipherData,nPos,encbuffer,0,16);

            SEED_KISA.SeedDecrypt(encbuffer, pdwRoundKey, decbuffer);

            ByteUtil.bytecopy(decbuffer,0, bytePlainData, nPos, 16);
        ...
        if(nRemain > 0)
        {
            ByteUtil.bytecopy(byteCipherData,nPos,encbuffer,0,nRemain);
            SEED_KISA.SeedDecrypt(encbuffer, pdwRoundKey, decbuffer);
            ByteUtil.bytecopy(decbuffer,0, bytePlainData, nPos, nRemain);
        }

    }

    // User secret key
    static final byte pbUserKey[] = "KBSECQWSDFVBBNM}{PLKJNH".getBytes();

    /**
     * transaction 로그를 encrypt하고 encode한다.
     * @param plainLog
     * @return
     */
    public static byte[] ENCRYPT(byte[] plainLog)
    {
...
//        };
        ...
        byte pbData[] = plainLog;
//        {
//            (byte) 0x00, (byte) 0x01, (byte) 0x02, (byte) 0x03, (byte) 0x04, (byte) 0x05, (byte) 0x06, (byte) 0x07,
//            (byte) 0x08, (byte) 0x09, (byte) 0x0A, (byte) 0x0B, (byte) 0x0C, (byte) 0x0D, (byte) 0x0E, (byte) 0x0F
//        };
        // 2. 데이터 버퍼 준비
        int nBufferLength = pbData.length;
        final int nRemain = pbData.length % 16;
        if(nRemain != 0) nBufferLength += (16 - nRemain);

//        byte pbCipher[] = new byte[16];
        byte pbCipher[] = new byte[nBufferLength];
        byte pbPlain[] = new byte[nBufferLength];

        System.arraycopy(pbData, 0, pbPlain, 0, pbData.length);
        // Derive roundkeys from user secret key
        SEED_KISA.SeedRoundKey(pdwRoundKey, SEED_KISA.pbUserKey);
        SEED_KISA.encryptall(pbPlain, pdwRoundKey, pbCipher);
        String base64trlog = Base64Util.encode(pbCipher);
        return base64trlog.getBytes();
    }

    /**
     * transaction 로그를 decode하고 decrypt한다.
     * @param pbBase64
     * @return
     */
    public static byte[] DECRYPT(String pbBase64)
    {

//        KLogger.debug("DECRYPT base64\t: "+ pbBase64.trim());

        //decode base64
        byte[] pbCipher = Base64Util.base64Decode(pbBase64);


//        KLogger.debug("DECRYPT Cipher \t: "+ new String(pbCipher).trim());

        int pdwRoundKey[] = new int[32];

        // User secret key
//        byte pbUserKey[] = "KBSECQWSDFVBBNM}{PLKJNH".getBytes();

        // 2. 데이터 버퍼 준비
        int nBufferLength = pbCipher.length;
        final int nRemain = pbCipher.length % 16;
        if(nRemain != 0) nBufferLength += (16 - nRemain);
        ...
        // Decryption
        SEED_KISA.decryptall(pbCipher, pdwRoundKey, pbPlain);
    ...
    public static void main(String[] args){
        ...
        try {
            log.debug(new String(SEED_KISA.ENCRYPT("jdbc:mysql://192.168.0.171:3306/upts?useUnicode=true&autoReconnect=true&useSSL=false&characterEncoding=utf8&allowMultiQueries=true".getBytes())));
        } catch (Exception e) {
            e.printStackTrace();
        }
